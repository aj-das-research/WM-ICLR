...
def main():
    ...
    gs = fig.add_gridspec(2, 5, width_ratios=[1.3, 0.9, 0.9, 0.22, 1.35], height_ratios=[1, 1], wspace=0.06, hspace=0.32,
                          left=0.01, right=0.99, top=0.9, bottom=0.14)
    ...
    for r in range(2):
        ...
        for sp in az.spines.values():
            sp.set_visible(True); sp.set_edgecolor("#FFD23F"); sp.set_linewidth(1.2)
        if r == 0:
            az.set_title("zoom (yellow box)", fontsize=6.5, pad=2)
        ag = fig.add_subplot(gs[r, 2]); ag.imshow(fut, aspect="equal")
        from matplotlib.colors import LinearSegmentedColormap as _L
        gcm = _L.from_list("gate", [(0.9, 0.62, 0.0, 0.0), (0.9, 0.62, 0.0, 0.75)])
        ag.imshow(gate, cmap=gcm, extent=(-0.5, w - 0.5, h - 0.5, -0.5), aspect="equal", interpolation="bicubic", vmin=float(np.quantile(gate, 0.05)), vmax=float(np.quantile(gate, 0.95)))
        ag.set_xticks([]); ag.set_yticks([])
        if r == 0:
            ag.set_title("learned gate $g$", fontsize=6.5, pad=2)
        for a_ in (ax, ag):
            for sp in a_.spines.values():
                sp.set_visible(True); sp.set_edgecolor("#C9CED6"); sp.set_linewidth(0.6)
    ...
    ax.set_title("(b) error increase without transport", fontsize=6.6, pad=3, loc="right")
    ...
    ax.set_title("(c) gain over Direct, by motion decile", fontsize=6.6, pad=3, loc="right")
    ...
    ax.set_title("(d) actions steer moving parts", fontsize=6.6, pad=3, loc="right")
    fig.text(0.01, 0.955, "(a) Where ShiftWM beats Direct on held-out DROID", fontsize=7.4, fontweight="bold", color=mf.INK)
    from matplotlib.patches import Patch
    fig.legend(handles=[Patch(color=GREEN, alpha=0.8, label="ShiftWM lower error"), Patch(color=RED, alpha=0.8, label="Direct lower error"),
                        Patch(color=(0.9, 0.62, 0.0), alpha=0.75, label="higher gate (more transport)")],
               loc="lower left", bbox_to_anchor=(0.01, 0.0), ncol=3, fontsize=5.8, frameon=False, handlelength=1.2, columnspacing=1.2)
