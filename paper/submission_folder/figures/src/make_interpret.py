...
def main():
    ...
    gs = fig.add_gridspec(2, 5, width_ratios=[1.25, 0.72, 0.72, 0.08, 1.55], height_ratios=[1, 1], wspace=0.08, hspace=0.3,
    ...
    for r in range(2):
        ...
        ax = fig.add_subplot(gs[r, 0]); ax.imshow(fut, aspect="auto")
        ax.imshow(win_rgba(win, lim), extent=(-0.5, w - 0.5, h - 0.5, -0.5), aspect="auto", interpolation="bicubic")
        ...
        # zoom
        az = fig.add_subplot(gs[r, 1]); az.imshow(fut, aspect="auto")
        az.imshow(win_rgba(win, lim), extent=(-0.5, w - 0.5, h - 0.5, -0.5), aspect="auto", interpolation="bicubic")
        ...
        ag = fig.add_subplot(gs[r, 2]); ag.imshow(fut, aspect="auto")
        ag.imshow(gate, cmap="viridis", alpha=0.6, extent=(-0.5, w - 0.5, h - 0.5, -0.5), aspect="auto", interpolation="bicubic", vmin=0, vmax=1)
    ...
    ax.set_title("(b) error increase if transport is removed", fontsize=6.6, pad=2, loc="left")
    ...
    ax.set_title("(d) actions steer transport of moving parts", fontsize=6.6, pad=2, loc="left")
