...
\tikzset{badge/.style={circle, fill=#1, text=white, font=\tiny\bfseries, inner sep=0.6pt, minimum size=9pt}}
\begin{document}
...
\begin{scope}[x=0.92cm, y=0.98cm, shift={(-0.62cm,-0.55cm)}]
  ...
  \node[font=\scriptsize, text=addred, anchor=north west, align=left] at (0.95,6.28)
    {additive head: synthesise\\$\z_{0,\pi(i)}-\z_{0,i}$ from scratch};
  ...
  \node[font=\scriptsize, text=arorange, anchor=west, align=left] at (5.3,1.5) {recursive: errors\\feed forward};
  ...
  \node[font=\scriptsize, text=obsblue!85!black, anchor=north] at ($(pi)+(0.25,-0.16)$) {$\z_{0,\pi(i)}$};
  \draw[gateamber, line width=1pt, densely dotted] (zi) -- (T);
  \node[font=\scriptsize, text=gateamber!85!black, anchor=south, rotate=26] at ($(zi)!0.45!(T)+(0,0.02)$) {gate $g$};
