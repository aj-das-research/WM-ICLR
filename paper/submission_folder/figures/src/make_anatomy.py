...
def heat(ax, m, title, vmax, cbar=False, fig=None):
    ...
    ax.set_xticks([1, 5, 10]); ax.set_yticks([1, 10]); ax.set_yticklabels(["static", "fast"], fontsize=mf.FS_TICK)
    ax.tick_params(labelsize=mf.FS_TICK, length=2); ax.grid(False)
    ax.set_xlabel("horizon $k$", fontsize=mf.FS_NOTE, labelpad=1)
    lo, hi = m.min(), m.max()
...
def main():
    ...
    gs = fig.add_gridspec(1, 4, width_ratios=[1, 1, 1.02, 1.02], wspace=0.42, left=0.075, right=0.975, top=0.84, bottom=0.2)
    ...
    for j, (base, title) in enumerate((("direct", "(a) Gain over Direct (%)"), ("ar", "(b) Gain over AR (%)"))):
        ...
        labs = ax.clabel(cl, fmt="%d", fontsize=mf.FS_NOTE, inline=True, inline_spacing=2)
        for t_ in labs:                                            # drop labels that would be cut by the panel edge
            x_, y_ = t_.get_position()
            if not (1.6 < x_ < 9.4 and 1.5 < y_ < 8.6):
        ...
        ax.set_xticks([1, 5, 10]); ax.set_yticks([1, 10]); ax.set_yticklabels(["static", "fast"] if j == 0 else ["", ""], fontsize=mf.FS_TICK)
        ax.tick_params(labelsize=mf.FS_TICK, length=2); ax.grid(False)
        ax.set_xlabel("horizon $k$", fontsize=mf.FS_NOTE, labelpad=1)
        if j == 0:
            ax.set_ylabel("true motion (decile)", fontsize=mf.FS_NOTE, labelpad=0)
        ax.set_title(title, fontsize=6.8, pad=5)
        if m.min() > 0:
            ax.text(0.97, 0.96, f"+{m.min():.0f}% to +{m.max():.0f}% ($\\star$ max)", transform=ax.transAxes, fontsize=mf.FS_NOTE, color=mf.INK, ha="right", va="top",
    ...
    ax.stackplot(k, g, 1 - g, colors=[GREEN, "#D5DAE1"], alpha=0.9, edgecolor="white", linewidth=0.6)   # keep = grey (copy)
    ...
    ax.text(9.8, c["static"]["gate"][-1] - 0.06, "gate on static patches", fontsize=mf.FS_NOTE, color="white", ha="right", va="top")
    ax.set_xlim(1, 10); ax.set_ylim(0, 1); ax.set_xticks([1, 5, 10]); ax.tick_params(labelsize=mf.FS_TICK, length=2); ax.grid(False)
    ax.set_xlabel("horizon $k$", fontsize=mf.FS_NOTE, labelpad=1); ax.set_ylabel("share of forecast", fontsize=mf.FS_NOTE, labelpad=1)
    ax.set_title("(c) Moving parts are moved", fontsize=6.8, pad=5)
    ...
    for a, lab, col in (("ar", "AR", ORANGE), ("direct", "Direct", BLUE), ("shiftwm", "ShiftWM", GREEN)):
        ...
        ax.text(10.25, rec[a][-1] + dy * 1.4, f"{rec[a][-1]:.0f}%", fontsize=mf.FS_NOTE, va="center", color=col,
                fontweight="bold" if a == "shiftwm" else "normal")
    ax.set_xlim(0.7, 11.6); ax.set_ylim(20, 90); ax.set_xticks([1, 5, 10]); ax.tick_params(labelsize=mf.FS_TICK, length=2)
    ax.set_xlabel("horizon $k$", fontsize=mf.FS_NOTE, labelpad=1); ax.set_ylabel("recovered (%)", fontsize=mf.FS_NOTE, labelpad=1)
    ax.set_title("(d) Share of oracle gain", fontsize=6.8, pad=5)
    ax.legend(fontsize=mf.FS_NOTE, frameon=False, loc="lower right", handlelength=1.2, borderaxespad=0.3)
    mf.qa(fig, "anatomy", 5.5)
