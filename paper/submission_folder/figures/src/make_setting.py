...
def block(fig, gs, ds, name, letter):
    ...
    sub = gs.subgridspec(2, 8, width_ratios=[1, 1, 1, 0.22, 1, 1, 1, 1.0], height_ratios=[1, 0.95], hspace=0.5, wspace=0.05)
    ...
    for i, (c, im) in enumerate(zip(cols, ims)):
        ...
        ax.set_title(labels[i], fontsize=6.4, pad=1.5, color=FUT if future else OBS)
        if i == 0:
            ax.text(0.0, 1.32, f"({letter}) {name}", transform=ax.transAxes, fontsize=7.2, fontweight="bold", color=mf.INK)
        if i == 1:
            ax.text(0.5, -0.2, "observed (input)", transform=ax.transAxes, fontsize=6.0, ha="center", va="top", color=OBS)
        if i == 4:
            ax.text(0.5, -0.2, "future frames (to predict)", transform=ax.transAxes, fontsize=6.0, ha="center", va="top", color=FUT)
    ...
    ax.set_xticks([]); ax.set_yticks([])
    ax.set_title("change to $t{+}10$", fontsize=6.4, pad=1.5)
    ax.text(0.03, 0.04, f"25% of patches: {share:.0f}%", transform=ax.transAxes, fontsize=5.3, color="white",
            fontweight="bold", bbox=dict(fc="#1F2A37", ec="none", alpha=0.65, pad=0.8))
    ...
    for j, (c, lab) in enumerate(zip(["#C0392B", "#1E8449", "#2F6DB5"], "xyz")):
        ax.plot(steps - t0, xyz[:, j], color=c, lw=1.1, marker="o", ms=2.2, label=lab)
    ...
    ax.text(-1.45, 0.94, "past actions", transform=ax.get_xaxis_transform(), fontsize=5.6, ha="center", va="top", color=OBS)
    ax.text(4.5, 0.94, "future actions $\\mathbf{a}_{0:K-1}$ (given to the model)", transform=ax.get_xaxis_transform(),
            fontsize=5.6, ha="center", va="top", color=mf.INK)
    ax.set_xlim(-2.4, K - 0.6); ax.set_xticks(range(-2, K)); ax.set_xticklabels([f"{s:+d}" if s else "0" for s in range(-2, K)], fontsize=5.4)
    ax.tick_params(axis="y", labelsize=5.4); ax.set_ylabel("end-effector", fontsize=5.6, labelpad=1)
    ax.set_xlabel("model step (1 step = 1/3 s)", fontsize=5.8, labelpad=1)
    ax.legend(fontsize=5.4, ncol=3, frameon=False, loc="lower left", handlelength=1.2, columnspacing=0.8)
...
def main():
    ...
    gs = fig.add_gridspec(2, 1, hspace=0.5, left=0.085, right=0.985, top=0.93, bottom=0.08)
