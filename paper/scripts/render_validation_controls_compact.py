...
PACK = ROOT / 'paper/figure_sources/validation_controls_compact'
...
plt.rcParams.update({'font.family':'Liberation Sans','font.size':8,'mathtext.fontset':'stix',
                     'pdf.fonttype':42,'svg.fonttype':'none','svg.hashsalt':'historical-validation-controls-v1',
...
def footer(fig):
    label(fig,198,157,r'Δ standardized MSE ($\times10^{-3}$); left favors the first named model.',ha='center')
    label(fig,198,173,'Secondary: context h5 mean −0.116% gain (CI crosses zero).',ha='center',color=AMBER)
def audit(fig):
    ...
    return {'status':'passed' if not issues else 'needs_revision','issues':issues,'minimum_font_pt':8,'size_inches':[5.5,2.5]}
def export(fig,stem):
    quality=audit(fig)
...
def composition_a(data):
    ...
    label(fig,7,12,'Historical context model (ours) · DROID validation',size=8.7)
    ...
    for x,title,sub in headings:
        label(fig,x,31,title,size=8.5)
        label(fig,x,44,sub,color=GRAY)
    label(fig,7,56,'Endpoint',color=GRAY)
    label(fig,140,56,'Mean h10; last row h5',color=GRAY)
    label(fig,274,56,'Both h10-trained',color=GRAY)
    # Panel a retains all six effects, paired by arm and differentiated by horizon.
    a=axis(fig,54,128,66,136,(-2.1,6.0),[-2,0,2,4,6],6)
    for i,r in enumerate(data['panels'][0]['rows']):
        y=66+(i+.5)*70/6
        label(fig,7,y,f"{r['label']} {r['horizon']}")
        dot(a,r,i,'o' if r['horizon']==5 else 'D')
    b=axis(fig,200,264,66,136,(-3.05,.8),[-3,-2,-1,0],5)
    ...
    c=axis(fig,325,390,66,136,(-1.5,.15),[-1.5,-1,-.5,0],4)
    for i,r in enumerate(data['panels'][2]['rows']):
        label(fig,274,66+(i+.5)*70/4,r['label'])
        dot(c,r,i,'o' if 'mean' in r['label'] else 'D')
    footer(fig)
    return export(fig,'validation_controls_compact')
...
def main():
    ...
    img=Image.open(HERE/'validation_controls_compact.png').convert('RGB')
    img.save(HERE/'validation_controls_compact_paper_width.png')
    img.convert('L').convert('RGB').save(HERE/'validation_controls_compact_grayscale.png')
    ...
    evidence={'status':'passed','scope':data['scope'],'sources_sha256':sources,'layout':quality,
              ...
              'exports_sha256':{ext:sha(HERE/('validation_controls_compact.'+ext)) for ext in ['pdf','svg','png']}}
    (HERE/'validation_controls_compact.json').write_text(json.dumps(evidence,indent=2,sort_keys=True)+'\n')
