...
PUBLIC=ROOT/'paper/figure_sources/composition_compact'
...
class Canvas:
 ...
 def render(self,stem):
  ...
  self.native(PUBLIC/'composition_compact.drawio')
...
def draft_a():
 ...
 for i,x in enumerate([5,49,93,137]):
  c.photo(f'pusht_v{i}.png',x,18,30);c.text(x+15,55,f'v{i}',8,VIOLET if i==3 else INK,'center')
  c.text(x+15,65,['Canonical','Warm','Cool','Dim'][i],8,MUTED,'center')
 c.photo('reacher.png',196,18,38);c.text(215,65,'Reacher',8,INK,'center')
 c.text(5,76,'Physical setting',8.5,INK,bold=True)
 for j,x in enumerate([114,151,189,229]):c.text(x,76,f'p{j}',8,VIOLET if j==3 else INK,'center')
 for y,label,vals in [(88,'PushT damping',['0','.15','.65','.95']),(100,'Reacher density',['1000','650','1350','1500'])]:
  c.text(5,y,label,8,MUTED)
  for j,x in enumerate([114,151,189,229]):c.text(x,y,vals[j],8,VIOLET if j==3 else INK,'center')
 c.line([(250,5),(250,101)],LINE,.65)
 c.text(274,9,'Cross the two factors',8.5,INK,bold=True)
 matrix(c,284,26,22,True,False,16)
 c.text(328,98,'Test: all 9 in-range',8,MUTED,'center')
...
def main():
 ...
 c=draft_a();g=c.render(OUT/'composition_compact')
 ...
 evidence={'status':'passed_source_and_layout_checks','scope':p['scope'],
  'canvas_inches':[W/72,H/72],'aspect_ratio':'21:6','font_minimum_pt':8,
  ...
  'outputs':{str(path.relative_to(ROOT)):sha(path) for path in [OUT/'composition_compact.pdf',OUT/'composition_compact.svg',OUT/'composition_compact.png',PUBLIC/'composition_compact.drawio',PUBLIC/'geometry.json']}}
 (OUT/'composition_compact.json').write_text(json.dumps(evidence,indent=2)+'\n')
 print(json.dumps({'pdf_sha256':sha(OUT/'composition_compact.pdf'),'renderer_sha256':sha(__file__)}))
