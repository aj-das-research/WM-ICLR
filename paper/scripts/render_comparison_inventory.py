...
def main():
    ...
    droid_complementary_sources = []
    droid_display = ROOT / 'paper/generated/metric_completion_droid/evidence.json'
    if droid_display.exists():
        display = json.loads(droid_display.read_text())
        if display.get('status') != 'completed':
            raise ValueError('Complementary DROID display is incomplete')
        for name, expected in display['sources'].items():
            path = ROOT / name
            if hashlib.sha256(path.read_bytes()).hexdigest() != expected:
                raise ValueError('Stale complementary DROID display source: ' + name)
            droid_complementary_sources.append(path)
        droid_complementary_sources.append(droid_display)
        coverage[0] = ('DROID', 'Recorded robot forecasting', 'MSE / MAE / L1 / cosine; h1--h10',
                       '12 predictors; development', r'\ref{tab:droid-complementary-metrics}')
    external = [
    ...
    if external_ready:
        external[5:6] = [
            ('DROID', 'Adapted official DINO-WM', 'MSE / MAE / L1 / cosine',
             r'Complete: four recipes, three seeds each; App.~\ref{app:external-dinowm}'),
            ('DROID', 'LeWM', 'Feature MSE by horizon', 'Not run under current forecast contract'),
        ]
    new_sources = []
    for relative, expected in [
        ('reports/metrics_completion_v1/planning/finalization.json', 'complete'),
        ('reports/iclr_review_2026-09-22/rgb_endpoint_v2/finalization.json', 'complete'),
    ]:
        path = ROOT / relative
        record = json.loads(path.read_text())
        if expected is not None and record.get('status') != expected:
            raise ValueError('Incomplete comparison: ' + relative)
        new_sources.append(path)
    coverage[4:4] = [
        ('Spatial PushT', 'Closed-loop goal reaching', 'Success; physical error; cost', '4 arms; 3 seeds; 100 paired cases', r'\ref{tab:current-spatial-planning-success}'),
        ('Spatial Reacher', 'Closed-loop goal reaching', 'Success; physical error; cost', '4 arms; 3 seeds; 100 paired cases', r'\ref{tab:current-spatial-planning-success}'),
        ('IWS decoded RGB', 'Recorded-video endpoints', 'MSE / PSNR / SSIM / LPIPS / UIQI', '4 learned arms + persistence references', r'\ref{tab:rgb-endpoint-comparison}'),
    ]
    external[-1] = ('IWS PushT / Box / Rope', 'RLA-WM', 'Common RGB / feature protocol',
                    'Pending; gated backbone access requested; representation differs')
...
"""
    ...
    manifest['sources'].update({str(p.relative_to(ROOT)): hashlib.sha256(p.read_bytes()).hexdigest()
                                for p in droid_complementary_sources})
    manifest['sources'].update({str(p.relative_to(ROOT)): hashlib.sha256(p.read_bytes()).hexdigest()
    ...
    print(json.dumps({"status": "rendered", "comparison_scopes": len(coverage)-1, "input_audits": 1}))
