...
Final model test evaluation & Not run & Not run \\
\bottomrule
\end{tabular}
...
final tests remain gated by the registered development criteria and are not measured outcomes.}
...
both domains, with the six subsequent wide-gain runs in a separate group.
No task succeeds during the ten-command common support.
...
\IfFileExists{generated/simulator_tables/extensions_complete.tex}{%
\input{generated/simulator_tables/extensions_complete.tex}}{%
\input{generated/extensions_completed/all_development.tex}}
\caption{\textbf{Historical context model: complete extension scorecard.}
The original 36 models and six wide-gain follow-ups are grouped by domain,
backbone and recipe. S0/1/2 gives success counts out of eight physical tasks
per training seed; 24 task--seed instances are not 24 independent tasks.
MSE averages three seeds on the same 16 development trajectories and
144 windows per domain. Black bold marks each group's best point mean.
Named-reference gains are success percentage points and relative h5 error
reductions; green bold marks favorable point differences, not significance.
All 42 runs completed 30 epochs and both evaluations. Paired uncertainty
is retained separately. Final model tests and comparable physical-error
aggregates remain pending. These are historical-model development results.}
...
\centering\small
\setlength{\tabcolsep}{4pt}
\input{generated/extensions_completed/paired_development.tex}
...
\paragraph{What the tissue failures record.}
An audit of all 18 saved tissue policy runs retains 144 method--seed--task
instances on eight goals. Only 12 succeed; all 132 failures never enter
the 2\,mm target region. Forty-one stop on an invalid action and 91 exhaust
the budget, with no recorded unstable-deformation steps
(Table~\ref{tab:extension-tissue-failure-audit}). Only 1,816 of 4,563 complete
five-command decisions reduce target distance. These descriptive traces
identify target approach and command feasibility as unresolved issues;
they do not measure counterfactual candidate ranking or establish a unique
cause of failure.
\input{generated/extensions_completed/tissue_failure_audit_20260920.tex}
...
These are subsequent comparisons on the same development tasks, and
forecast MSE changes little. The completed fixed-goal diagnostic now
compares all twelve narrow/wide checkpoints on the same 16 goals and
120 pairs (Table~\ref{tab:extension-geometry-diagnostic}). Widening retains
only 16.04--16.25\% of canonical pairwise separation, slightly less than
the narrow models in every matched comparison. Physical-distance rank
correlation improves by just 0.0031--0.0046, and attained wide gains span
0.803--1.229 rather than approaching the permitted extremes.
Thus increased gain capacity alone did not restore this goal geometry.
The observed success increases do not demonstrate improved action ranking
or independent generalization, and do not justify final-test promotion.

\input{generated/extensions_completed/geometry_diagnostic_20260920.tex}

\begin{table}[H]
\centering\small
\setlength{\tabcolsep}{4pt}
\IfFileExists{generated/simulator_tables/geometry_effects.tex}{%
\input{generated/simulator_tables/geometry_effects.tex}}{%
\input{generated/extensions_completed/geometry_ablation.tex}}
...
The six revised models use seeds 0, 1 and 2. Absolute scores are consolidated
in Table~\ref{tab:extension-drone}; this table retains
...
\input{generated/extensions_completed/official_adajepa.tex}
