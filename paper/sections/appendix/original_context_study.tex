...
Weights remain fixed online. This historical model combines shared observation
calibration, corrected-transition dynamics inference and paired supervision;
the LeWM backbone and CEM planner are reused. Equations~\ref{eq:obs}--\ref{eq:cost}
specify these computations. Appendix~\ref{sec:implementation} gives dimensions,
initialization and parameter counts. The current spatial decoder is defined
separately in Section~\ref{sec:editorial-method} and its consolidated
architecture in Figure~\ref{fig:editorial-spatial-method}.
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{generated/editorial/composition_compact.pdf}
\caption{\textbf{Crossed appearance and dynamics in the historical simulation study.} Full recorded simulator frames illustrate PushT and Reacher. The four PushT views apply the exact RGB transforms to one state; physical settings change damping or arm and finger density together. Training and ordinary validation use seven pairs; $(v_1,p_1)$ is development and $(v_2,p_2)$ is held-out composition. Three additional pairs use new factor values; blank cells are unevaluated. Independent test trajectories cover all nine in-range pairs, including familiar combinations. Trajectory seeds are split before paired rendering; factor IDs are unavailable to the deployed model. This is the historical context-model protocol.}
\label{fig:split}
\end{figure}
...
\IfFileExists{generated/editorial/forecast_landscape_v1.pdf}{
\input{generated/editorial/forecast_landscape_v1_figure.tex}
