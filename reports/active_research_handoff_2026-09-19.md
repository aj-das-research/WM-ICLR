...
IWS real PushT feature preparation completed on GPU job200456: 600 recordings /119,887 native frames, split into 480 internal training /120 development recordings. Full package validation passed. No official validation payload was read, no IWS predictor has been trained/evaluated yet. Cache path: `data/features/iws_pusht_spatial_v1`; receipt: `reports/evidence/iws_pusht_cache_completion.json`. Two independently reviewed, separately registered full caches are now running: Box200465 onws-l1-006 and Rope200466 onws-l5-004 (1GPU/8CPU/24Geach;602recordings each). They preserve native command widths14/8 and use only the upstream training partition/internal split. No IWS predictor training or official evaluation has started. Next implementation is the single-observation matched protocol in `reports/real_video_development/iws_single_observation_design.md`; register the full training/evaluation contract before launching predictors.
