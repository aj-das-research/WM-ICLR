"""The ShiftWM forecaster and its matched baselines on frozen patch features.

This module is a verbatim copy of the research implementation (``V2WorldModel``); only names,
type hints, docstrings and the checkpoint loaders were added, the arithmetic is unchanged.

Architecture (shared by all learned arms):

* ``MemoryEncoder``: self-attention over the ``H`` observed frames' patch tokens
  (+ frame, position and past-action embeddings).
* ``QueryDecoder``: ``N`` query tokens per future step, initialised from the last observed grid;
  self-attention, cross-attention to memory, AdaLN-zero conditioning on the causal
  action-prefix state (GRU) and a horizon embedding.

Output heads ("arms"):

* ``shiftwm``    : ``(1-g) Z0 + g * LocalTransport(Z_{-S+1..0}) + s * W h``  (ours, default)
* ``direct``     : ``Z0 + W h``
* ``ar``         : residual one-step predictor rolled out recursively
* ``ar_tf``      : teacher-forced one-step predictor of absolute features, rolled out
* ``persistence``, ``linear`` : parameter-free references

All tensors live in (train-)standardised feature space with layout ``[B, frames, N=G*G, C]``.
"""
...
class WorldModelConfig:
    """Hyper-parameters of :class:`WorldModel` (same fields and defaults as the research code)."""
...
class WorldModel(nn.Module):
    """ShiftWM forecaster: ``forward(hist, past_actions, future_actions) -> [B, K, N, C]``.

    Args:
        config: a :class:`WorldModelConfig` or a dict of its fields (e.g. the ``"config"`` entry of a
            checkpoint). ``arm="shiftwm"`` (default) gives the transport forecaster of the paper.
    """

    def __init__(self, config):
        super().__init__()
        self.config = c = WorldModelConfig(**config) if isinstance(config, dict) else config
    ...
    # ------------------------------------------------------------------ forward
    def forward(self, hist: torch.Tensor, past_actions: torch.Tensor, future_actions: torch.Tensor,
                return_details: bool = False, teacher: "torch.Tensor | None" = None):
        """Forecast ``K`` future feature grids.

        Args:
            hist: observed features ``[B, H, N, C]`` (standardised).
            past_actions: actions between observed frames ``[B, H-1, A]``.
            future_actions: planned actions ``[B, K, A]``.
            return_details: (``shiftwm`` arm) also return ``{"weights", "gate", "correction"}``.
            teacher: ground-truth frames for teacher forcing (``ar_tf`` training only).

        Returns:
            ``[B, K, N, C]`` predictions, or ``(pred, details)`` if ``return_details``.
        """
        c = self.config
    ...
    # ------------------------------------------------------------------ loaders
    @classmethod
    def from_config(cls, config: Union["WorldModelConfig", Mapping[str, Any], None] = None, **overrides: Any) -> "WorldModel":
        """Build a freshly initialised model from a config (dict or dataclass) plus keyword overrides."""
        if config is None:
            fields = {}
        ...
        return cls(WorldModelConfig(**fields))

    @classmethod
    def from_checkpoint(cls, path: Union[str, os.PathLike], map_location: Any = "cpu", strict: bool = True) -> "WorldModel":
        """Load a checkpoint saved as ``{"model": state_dict, "config": dict, ...}`` and return it in eval mode."""
        ckpt = torch.load(path, map_location=map_location, weights_only=False)
        if "model" not in ckpt or "config" not in ckpt:
            raise KeyError(f"{path}: expected keys 'model' and 'config', found {sorted(ckpt)}")
...
# Backwards-compatible aliases (names used in the research code).
V2Config = WorldModelConfig
V2WorldModel = WorldModel
ShiftWM = WorldModel
