...
spec = importlib.util.spec_from_file_location("reserved_finalization_tests", ROOT / "scripts/real_video_iws_reserved_v1/finalize.py")
...
def test_receipt_reconstruction_rejects_aggregate_tamper(monkeypatch, tmp_path):
    ...
    src = tmp_path / "scripts/real_video_iws_reserved_v1/evaluate.py"; src.parent.mkdir(parents=True); src.write_text("synthetic source")
    ...
    receipt = {**row["receipt"], "schema": ev.SCHEMA, "status": "passed", "scope": ev.SCOPE,
               **{k: row[k] for k in ("name", "task", "mode", "seed", "package_kind", "selected_epoch")},
               ...
               "backend": {"device": "cpu", "dtype": "float32", "threads": 8, "interop_threads": 1, "batch_size": 64, "autocast": False, "tf32": False},
