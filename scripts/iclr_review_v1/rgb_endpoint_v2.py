...
OUT=ROOT/'reports/iclr_review_2026-09-22/rgb_endpoint_v2'
...
def register():
    ...
    document={'schema':'posthoc_iws_rgb_endpoint_v2','status':'frozen_before_rgb_endpoint_scoring',
      ...
      'forecast_runtime':'existing CPU FP32 row-wise GRU,8 threads,interop1; prior endpoint MSE checked at rtol2e-5/atol2e-6, matching existing DROID complementary-metric tolerance; exact discrepancies retained',
      ...
      'numerical_revision':{'original_protocol_sha256':sha(ROOT/'reports/iclr_review_2026-09-22/rgb_endpoint/registration.json'),'diagnostic_sha256':sha(ROOT/'reports/iclr_review_2026-09-22/rgb_replay_diagnostic.json'),'reason':'exact-MSE gate failed for7/200 rope bounded seed2 windows; maximum absolute difference5.96e-8; same-model replay exact; timer typo at finalization also repaired; all45 rows recomputed, no v1 accuracy reused'},
...
def run(task):
    ...
    output=OUT/task;output.mkdir(exist_ok=True);began=time.monotonic();rows=[];replay_checks=[]
    ...
    for row in [r for r in reg['runs']if r['task']==task]:
        ...
        with np.load(ROOT/'reports/real_video_iws_reserved_recovery_v2/evaluations'/(row['name']+'.npz'),allow_pickle=False)as original:
            actual=np.concatenate(errors);expected=original['standardized_mse'][:,-1]
            np.testing.assert_allclose(actual,expected,rtol=2e-5,atol=2e-6)
            replay_checks.append({'name':row['name'],'exact':bool(np.array_equal(actual,expected)),'max_absolute_difference':float(abs(actual-expected).max()),'rtol':2e-5,'atol':2e-6})
    ...
    atomic_json({'status':'complete','task':task,'registration_sha256':sha(REG),'rows':rows,'metric_names':list(METRICS),
        ...
        'elapsed_seconds':time.monotonic()-began,'original_feature_mse_checks':replay_checks},OUT/(task+'.json'))
