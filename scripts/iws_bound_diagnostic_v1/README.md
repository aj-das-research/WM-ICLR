...
That cross-check alone allows `32*eps64*(1+abs(target_z)+max(abs(lo),abs(hi))+
abs(mean/std))` to account for cancellation in reconstructing raw endpoints at
small training standard deviations; it does not alter either reported distance.
