...
class LoaderBoundaryTests(unittest.TestCase):
    def setUp(self):
        ...
        self.document = {"schema": "iws_local_inference_bundle_v1", "package_kind": runtime.KIND,
    ...
    def test_complete_identity_manifest(self):
        self.assertEqual(len(runtime.verify_bundle(self.root)["models"]), 27)
    ...
    def test_omitted_source_hash_rejected(self):
        del self.document["files"]["src/shiftwm/real_video_iws/model.py"]; self.commit()
        with self.assertRaises(ValueError):
            runtime.verify_bundle(self.root)
