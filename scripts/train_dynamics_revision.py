#!/usr/bin/env python3
"""Train only a new dynamics residual over a completed frozen framewise donor."""
...
from shiftwm.dynamics_revision import (DynamicsRevision, completed_framewise_source,
                                      file_sha256, load_revision_package, validate_donor_state)
...
def initialize(config):
    ...
    provenance["revision_implementation_hashes"]["train_dynamics_revision.py"] = file_sha256(__file__)
    provenance["revision_environment"] = environment
    return DynamicsRevision(donor, config.get("revision"), provenance)
...
def validate_completed(output, identity):
    ...
    load_revision_package(output / "best", device="cpu")
    return summary
...
def fit(model, config, train_set, val_set):
    ...
    metadata = {"config": config, "training_identity": identity,
                ...
                "selection": "minimum_validation_prediction_loss_over_completed_training_epochs",
