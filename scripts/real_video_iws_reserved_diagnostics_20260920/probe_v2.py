...
EVALUATOR = ROOT/'scripts/real_video_iws_reserved_v1/evaluate.py'
spec = importlib.util.spec_from_file_location('_unchanged_reserved_loader',EVALUATOR)
...
def diagnose(model,initial,commands):
    if torch.is_inference_mode_enabled() or any(p.is_inference() for p in model.parameters()) or initial.is_inference() or commands.is_inference():
        raise ValueError('Diagnostic must match evaluator normal model/input allocation')
    with torch.inference_mode(),torch.autocast(device_type='cpu',enabled=False):
        # Original make_batch creates from_numpy tensors inside inference_mode.
        return diagnose_inference(model,torch.from_numpy(initial.numpy()),torch.from_numpy(commands.numpy()))


def diagnose_inference(model,initial,commands):
    ...
    native_prefixes={h:model.predict(initial,commands[:,:h]) for h in PREFIXES}
    contiguous_prefixes={h:model.predict(initial,commands[:,:h].contiguous()) for h in PREFIXES}
    ...
    for horizon in PREFIXES:
        ...
        prefix=native_prefixes[horizon]
        contiguous=contiguous_prefixes[horizon]
...
def run(output,threads=8,batch_size=64):
    ...
    source_hashes={'probe':sha(__file__),'prefix_backend':sha(HELPER),'original_evaluator_loader':sha(EVALUATOR)}
    ...
    for seed in SEEDS:
        ...
        model,state=evaluation.load_selected(ROOT,row)
        model.eval()
    ...
    if checked_registration(ROOT)!=registry or source_hashes!={'probe':sha(__file__),'prefix_backend':sha(HELPER),'original_evaluator_loader':sha(EVALUATOR)}:raise ValueError('Sources changed during probe')
    ...
    document={'schema':'reserved_prefix_input_only_diagnostic_20260920_v2','status':'completed','completed_utc':datetime.now(timezone.utc).isoformat(),
              ...
              'allocation_contract':'Model allocated outside inference_mode; input tensor views and predictions inside inference_mode, matching original make_batch and scoring contexts',
              'supersedes_diagnostic_only':'probe_cpu8.json did not match evaluator inference-tensor allocation; retained unchanged',
