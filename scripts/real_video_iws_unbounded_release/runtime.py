...
def verify_bundle(root):
    ...
    if (manifest.get("schema") != "iws_unbounded_local_inference_bundle_v1"
    ...
    expected = {f"{t}_{m}_s{s}" for t in ("pusht", "bimanual_box", "bimanual_rope")
                for m in ("unbounded_spatial_mix",) for s in range(3)}
    rows = manifest.get("models", [])
    if len(rows) != 9 or {r.get("name") for r in rows} != expected:
        raise ValueError("All 9 registered models are required")
...
def load_model(directory, bundle_root=None):
    ...
    import torch
    from shiftwm.real_video_iws_unbounded import model as module
    if Path(module.__file__).resolve() != root / "src/shiftwm/real_video_iws_unbounded/model.py":
...
def verify_offline(root, output, forbid_root=None):
    ...
    if forbid_root is not None:
        forbidden = Path(forbid_root).resolve()
        ...
        def audit(event, args):
            if event == "open" and isinstance(args[0], (str, bytes)):
                ...
                if path.is_relative_to(forbidden) and not path.is_relative_to(environment):
                    denied_reads.append(str(path))
                    raise RuntimeError("Original workspace access forbidden during relocation proof")
    ...
    if Path(output).resolve().is_relative_to(root):
        raise ValueError("Parity receipts must remain outside the immutable bundle")
    manifest = verify_bundle(root)
    results = []
    ...
    if attempts or denied_reads:
        raise ValueError("A network or original-workspace read was attempted")
    ...
    result = {"status": "passed", "schema": "iws_unbounded_relocated_cpu_parity_v1",
              ...
              "original_workspace_reads_denied": forbid_root is not None,
              "original_workspace_read_attempts": len(denied_reads),
...
if __name__ == "__main__":
    ...
    verify_offline(args.bundle, args.output, args.forbid_root)
