...
Matched horizon-ten control; the reused architecture is unchanged.
Checkpoint selection averages all windows and all ten recursively predicted query
frames in fixed training-standardized coordinates, preserving original weighting. No test images enter training or selection.
...
PACKAGE_KIND = "shiftwm_real_video_droid_horizon10_v1"
TRAIN_HORIZON = 10
AGGREGATION = "window_weighted_mean_all_ten_recursive_query_standardized_mse"
...
def source_files():
    relative = ["scripts/real_video_development/horizon10_train.py",
                "scripts/real_video_development/horizon10_campaign.py",
                "scripts/real_video_development/horizon10_train.slurm",
...
def epoch_pass(model, loader, device, optimizer=None, bf16=False, grad_clip=1.):
    """All ten recursive targets; FP32 validation preserves window weighting.

    Selection uses the original window-weighted fixed-coordinate objective,
    extended from five to ten targets. An auxiliary equal-episode diagnostic is
    recorded but never selects a checkpoint. The reused predictor is causal.
    """
    training = optimizer is not None
    model.train(training)
    total, elements, batches, windows = 0., 0, 0, 0
    by_episode = {}
    with (nullcontext() if training else torch.inference_mode()):
        for batch in loader:
            batch = {key: value.to(device) if torch.is_tensor(value) else value for key, value in batch.items()}
            if batch["features"].shape[1] != 13 or batch["actions"].shape[1] != 12:
                raise ValueError("Horizon-ten training/validation requires three support plus ten query frames")
            if not training and "episode_index" not in batch:
                raise ValueError("Auxiliary equal-episode validation requires episode identities")
            if training:
                optimizer.zero_grad(set_to_none=True)
            with torch.autocast(device_type=device.type, enabled=training and bf16 and device.type == "cuda",
                                dtype=torch.bfloat16):
                out = model(batch)
                prediction, target = out["standardized_predictions"], out["standardized_targets"]
                if prediction.shape != target.shape or target.shape != batch["features"][:, 3:].shape:
                    raise ValueError("Prediction/target must contain every one of the ten query steps")
                if not training and (prediction.dtype != torch.float32 or target.dtype != torch.float32):
                    raise ValueError("Selection requires float32 forecast and target tensors")
                errors = (prediction.float() - target.float()).square()
                per_window = errors.mean(dim=(1, 2))
                mse = errors.mean()
            ...
            if training:
                ...
                if len(episode_indices) != len(per_window):
                    raise ValueError("Episode indices and validation windows disagree")
                for episode_index, value in zip(episode_indices, per_window.detach().cpu().tolist()):
                    if type(episode_index) is not int or episode_index < 0:
                        raise ValueError("Invalid validation episode index")
                    cumulative, count = by_episode.get(episode_index, (0., 0))
                    by_episode[episode_index] = (cumulative + value, count + 1)
            count = target.numel()
    ...
    return {"standardized_mse": metric, "elements": elements, "batches": batches,
            "windows": windows, "query_steps": TRAIN_HORIZON,
            "aggregation": "window_weighted" if training else AGGREGATION,
            **({} if training else {"episodes": len(by_episode),
                                   "auxiliary_equal_episode_standardized_mse": float(np.mean([v/n for v,n in by_episode.values()]))})}
    ...
    for row in rows:
        for part in ("train", "val"):
            if (not math.isfinite(row[part]["standardized_mse"]) or row[part]["standardized_mse"] < 0
                    ...
                    or row[part].get("query_steps") != TRAIN_HORIZON
                    or row[part].get("windows", 0) <= 0
                    or row[part].get("aggregation") != ("window_weighted" if part == "train" else AGGREGATION)
                    or (part == "val" and (row[part].get("episodes", 0) <= 0
                        or not math.isfinite(row[part].get("auxiliary_equal_episode_standardized_mse", float("nan")))
                        or row[part]["auxiliary_equal_episode_standardized_mse"] < 0))):
...
def fit(model, config, train_dataset, val_dataset, identity):
    ...
    if config.get("train_horizon") != TRAIN_HORIZON or config.get("validation_horizon") != TRAIN_HORIZON:
        raise ValueError("Explicit registered training and selection horizon must equal ten")
...
def train(config):
    ...
    expected_architecture = {"hidden_dim": 192, "depth": 4, "context_dim": 32, "context_hidden": 128}
    if (config.get("train_horizon") != TRAIN_HORIZON or config.get("validation_horizon") != TRAIN_HORIZON
            or config.get("model_config") != expected_architecture
            or config.get("lr") != 1e-4 or config.get("min_lr") != 1e-6
            or config.get("weight_decay") != .01 or config.get("batch_size") != 128
            or config.get("stride") != 2):
        raise ValueError("Horizon-ten matched-control architecture, optimizer, or horizon differs")
    ...
    train_data = RealVideoDataset(config["cache_root"], "train", horizon=TRAIN_HORIZON, stride=config.get("stride", 2), verify=True)
    if config.get("validation_stride", 5) != 5:
        raise ValueError("Registered validation stride is five stored frames")
    val_data = RealVideoDataset(config["cache_root"], "val", horizon=TRAIN_HORIZON, stride=5, verify=True)
