...
def run(name=None, index=None):
    ...
    with (directory / ".training.lock").open("w") as lock:
        ...
        execution = {"schema": "external_dinowm_raw_training_execution_v2", "status": "started", "name": name,
        ...
        if result["status"] != "completed":
            execution.update(status="epoch_boundary_continuation_requested", completed_epochs=result["completed_epochs"],
                             ended_utc=datetime.datetime.now(datetime.timezone.utc).isoformat())
            train.atomic_json(execution, receipt)
            job = os.environ.get("SLURM_JOB_ID", "")
            if not job.isdigit() or int(os.environ.get("SLURM_RESTART_COUNT", "0")) >= 5:
                execution.update(status="continuation_failed", continuation_error="Missing scheduler identity or five-restart limit exhausted")
                train.atomic_json(execution, receipt)
                raise RuntimeError("Full training incomplete; continuation requires reviewed Slurm resumption")
            try:
                requeue = subprocess.run(["scontrol", "requeue", job], check=True, capture_output=True, text=True)
            except BaseException as error:
                execution.update(status="continuation_failed", continuation_error=str(error))
                train.atomic_json(execution, receipt)
                raise
            execution.update(status="epoch_boundary_continuation_submitted", requeue_returncode=requeue.returncode,
                             requeue_job_id=job)
            train.atomic_json(execution, receipt)
            return result
        registry.verify()
        ...
        try:
            ...
            raise
        registry.verify()
        execution.update(status="completed", completed_epochs=100,
                         evaluation_elapsed_seconds=time.monotonic() - evaluation_began,
                         ended_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),
                         total_elapsed_seconds=time.monotonic() - began)
        train.atomic_json(execution, receipt)
        completion = {"status": "completed", "name": name, "epochs": 100,
