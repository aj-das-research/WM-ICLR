...
REPORT = ROOT / "reports/external_dinowm_raw_v2"
...
def expected_config(mode, seed):
    ...
    return {"mode": mode, "seed": seed, "model_config": model_module().ARCHITECTURE,
            "epochs": 100, "train_horizon": 10, "validation_horizon": 10,
            "lr": 5e-4, "min_lr": 5e-4, "weight_decay": .01, "batch_size": 32,
            "grad_clip": None, "bf16": False, "stride": 2, "validation_stride": 5,
            ...
            "protocol_path": "scripts/external_dinowm_raw_v2/README.md",
            "output_dir": f"runs/external_dinowm_raw_v2/{mode}_s{seed}"}
...
def sources():
    names = ["model.py", "train.py", "evaluate.py", "registry.py", "campaign.py", "run.slurm", "test_contract.py", "engine.py", "README.md"]
    ...
    paths += [ROOT / "external/official-dino-wm" / name for name in ("train.py", "conf/train.yaml", "models/visual_world_model.py", "datasets/traj_dset.py")]
...
def profile_gate():
    ...
    if {r["objective"] for r in result["rows"]} != {"official_one_step_shifted", "matched_recursive_h10"} or len(result["rows"]) != 2:
...
def register():
    ...
    value = {"schema": "adapted_official_dinowm_raw_droid_registration_v2", "status": "registered",
             "created_utc": datetime.datetime.now(datetime.timezone.utc).isoformat(),
             "expected_runs": 6, "expected_epochs_per_run": 100, "modes": list(MODES), "runs": runs,
             "scope": "Post-hoc corrected raw-coordinate baseline on original DROID train/development only; no reserved/test evaluation.",
             "adaptations": {"visual_coordinates": "raw pooled DINO tokens; feature statistics used only for scoring", "action_coordinates": "fixed training-normalized 35-D five-command blocks", "training_precision": "explicit FP32; upstream Accelerator precision unspecified", "recipe": "100 epochs/B32/constant AdamW5e-4/.01; no clipping", "profile_scope": "v1 BF16/B128 capacity prior, not an exact v2 FP32 runtime measurement"},
...
def verify(require_review=True):
    ...
    if (record.get("schema") != "adapted_official_dinowm_raw_droid_registration_v2"
            or record.get("status") != "registered" or record.get("expected_epochs_per_run") != 100
