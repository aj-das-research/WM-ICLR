...
# Source-bound scalar simulator traces for all 2,400 cases; no images, actions,
# neural weights or raw dataset payloads. Retain unfavorable trials and numeric
# reproducibility without relaxing the general per-file limit. Exact bytes only.
PUBLIC_LARGE_TEXT_PACKS = {
    "paper/figure_sources/current_spatial_planning_completion/data.json":
        "13c936745c16b15c4380db4e1cb03b685bb14d45e2f6d75c439d38648b029bec",
}
...
def main():
    ...
    for relative in sorted(candidates):
        ...
        if (str(relative) in PUBLIC_LARGE_TEXT_PACKS
                and hashlib.sha256(data).hexdigest() != PUBLIC_LARGE_TEXT_PACKS[str(relative)]):
            raise SystemExit(f"Reviewed scalar evidence pack changed: {relative}")
        if len(data) > 20_000_000 and str(relative) not in PUBLIC_LARGE_TEXT_PACKS:
