...
REGISTRATION = Path("configs/real_video_iws_reserved_v1/registration.json")
REPORT = Path("reports/real_video_iws_reserved_v1")
...
SCHEMA = "shiftwm_iws_reserved_evaluation_v1"
...
def checked_registration(root, registration_path=None):
    from shiftwm.real_video_iws_reserved.protocol import checked_registration as check
...
def evaluate(name, root=ROOT, registration_path=None):
    ...
    with output.with_suffix(".lock").open("a") as lock:
        ...
        cache = ReservedFeatureCache(root, row["task"], registration_path=registration_path)
        ...
        result = {"schema": SCHEMA, "status": "passed", "scope": SCOPE, "name": name,
                  **{k: row[k] for k in ("task", "mode", "seed", "package_kind", "selected_epoch")},
                  ...
                  "backend": {"device": "cpu", "dtype": "float32", "threads": 8, "interop_threads": 1, "batch_size": 64,
                              "autocast": False, "tf32": False, "torch": torch.__version__, "numpy": np.__version__},
                  ...
                  "prefix_policy": "Full-H60 curves remain unchanged; H15/30/45 use separately scored command-prefix calls."}
