#!/bin/bash
# V-JEPA 2-AC plug-in study, arm C (finetune_shiftwm): fine-tune the AC predictor + ShiftWM transport head on our DROID
...
# Run: sbatch -J vjepa_ft_C slurm_jebel/job.sbatch slurm_jebel/tasks/vjepa_ft_C.sh
...
for s in ${SEEDS:-0 1}; do
  python scripts/v2/vjepa_finetune.py --arm finetune_shiftwm --seed $s --cache $CACHE --checkpoint $CK --deadline $DEADLINE
  rc=$?
