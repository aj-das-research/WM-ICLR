...
from .data import (SPLITS, InternalInventory, canonical_hash, decode_native_rgb,
                   load_commands, read_json, require, sha, training_statistics, validate_arrays, task_width)


from shiftwm.real_video_iws.cache import (fsync_directory, atomic_json, writer_lock, establish_identity)


def expected_receipt(record, identity_sha256):
    return {"schema": "shiftwm_iws_task_native_episode_v1", "task": record["task"], "episode_id": record["episode_id"],
            "split": record["split"], "frames": record["frames"], "command_rows": record["frames"],
            "command_width": task_width(record["task"]), "feature_dim": 6144, "identity_sha256": identity_sha256,
...
def load_package(output, record, identity_sha256):
    ...
    with np.load(path, allow_pickle=False) as value:
        arrays = validate_arrays({key: value[key] for key in value.files}, record["frames"], task_width(record["task"]))
...
def write_package(output, record, identity_sha256, arrays, rgb):
    validate_arrays(arrays, record["frames"], task_width(record["task"]))
    ...
    try:
        ...
        with np.load(path, allow_pickle=False) as value:
            checked = validate_arrays({key: value[key] for key in value.files}, record["frames"], task_width(record["task"]))
...
def verify_inputs(inventory, records):
    ...
    for record in records:
        require(record.get("task") == inventory.task and record.get("command_width") == inventory.command_width, "Cache record task/width differs")
...
def finish_cache(output, records, identity_sha256, inventory, before_complete=None):
    ...
    for record in records:
        ...
        rows.append(receipt)
    index = {"schema": "shiftwm_iws_task_native_episode_index_v1", "status": "features_verified", "identity_sha256": identity_sha256, "episodes": rows}
    ...
    statistics = training_statistics(training_rows(), inventory.partitions["internal_train"], inventory.task)
    ...
    complete = {"schema": "shiftwm_iws_task_feature_cache_v1", "status": "complete", "task": inventory.task,
                ...
                "native_frames": sum(r["frames"] for r in records), "feature_dim": 6144, "command_width": inventory.command_width,
...
def build_cache(inventory, records, output, identity, encoder, batch_size=32, stop_after=None, before_complete=None):
    ...
    with writer_lock(output):
        require(identity.get("task") == inventory.task and identity.get("command_width") == inventory.command_width, "Extraction task/width identity differs")
...
class IWSFeatureCache:
    ...
    def __init__(self, output, inventory, records, expected_static_identity):
        ...
        require(all(r.get("task") == inventory.task and r.get("command_width") == inventory.command_width for r in records), "Loaded cache records use a different task/width")
        ...
        require(complete.get("schema") == "shiftwm_iws_task_feature_cache_v1" and complete.get("status") == "complete", "Cache is not complete")
        require(complete["task"] == inventory.task and complete["command_width"] == inventory.command_width, "Completed task/width differs")
        ...
        required_static = {"schema", "task", "command_width", "registration_sha256", "split_sha256", "input_manifest_sha256", "input_records_sha256",
