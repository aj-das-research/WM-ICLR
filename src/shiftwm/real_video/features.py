"""Pinned, frozen DINOv2 features of genuine recorded camera observations."""
import argparse
...
from .data import atomic_json, sha256, training_statistics, validate_manifest


def extract(data_root, output_root, encoder_root, batch_size=32, device="cuda"):
    import transformers
    ...
    identity = {"dataset_manifest_sha256": sha256(data_root / "manifest.json"),
                ...
                "pixel_normalization": "ImageNet mean/std", "pooling": "2x2 spatial mean of final patch tokens",
                "feature_dim": 1536, "precision": "bfloat16 encoder; float32 pooling/storage" if device == "cuda" else "float32",
                ...
                "camera_policy": "train/val/test exterior1; test exterior2 for natural camera transfer"}
    ...
    for episode in source["episodes"]:
            ...
            if receipt is None:
                ...
                with torch.inference_mode():
                    for start in range(0, len(images), batch_size):
                        ...
                        pieces.append(F.adaptive_avg_pool2d(grid,(2,2)).flatten(1).cpu().numpy())
        ...
        episodes.append(row)
        print(json.dumps({"event":"encoded", "episode":row["episode_id"], "count":len(episodes), "total":len(source["episodes"])}), flush=True)
    result = {"status":"complete", "dataset":"DROID selected real recordings", "identity": identity,
              "feature_dim":1536, "action_dim":35, "history_length":3,
...
if __name__ == "__main__":
    ...
    extract(args.data,args.output,args.encoder,args.batch_size,args.device)
